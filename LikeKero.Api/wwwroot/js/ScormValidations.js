function fValidateLookup(lVal, lLookup){
   ...
   if (!fIsUndefined(sRetVal)){
	...
	/*
		lstNodes = xmlLookup.selectNodes("//Lookup[@Type='" + lLookup + "']/Value[@Default='true']")
		if (lstNodes.length > 0){
			retTempVal = lstNodes(0).attributes.getNamedItem("InnerText").text
			return true;
		}
	*/
	
   var sRetVal = gScormLookups[lLookup + "__CheckFor"];
   if (!fIsUndefined(sRetVal)){
